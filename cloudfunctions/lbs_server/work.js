...
async function location(event) {
  ...
  try {
    ...
    if (result.formatted_addresses != null) {
      ...
      res.location = `${result.location.lat.toFixed(
        4
...
async function address(event) {
  try {
    const locres = await call({
      address: encodeURIComponent(
        event.address || "深圳市南山区深南大道10000号"
      ),
    }); // 传入编码后的地址，如果为空默认是腾讯大厦
    return await location({
      location: `${locres.location.lat},${locres.location.lng}`,
    }); // 如果正常返回，则找到坐标点了，传入经纬度信息方法，获取详细，上面的接口不管用
  } catch (e) {
    return {
      // 有接口其他问题，直接返回提示
      code: -1,
