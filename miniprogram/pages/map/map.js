...
Page({
      ...
      fail(e) {
        ...
        if (e.errMsg.indexOf("auth deny") !== -1) {
          ...
          wx.showModal({
            // 显示提示
            content:
              "你已经拒绝了定位权限，将无法获取到你的位置信息，可以选择前往开启",
            success(res) {
              if (res.confirm) {
                // 确认后
  ...
  async query() {
    if (that.data.model === 0) {
      ...
      if (location != null && location !== "") {
        ...
        if (!isNaN(lldata.latitude) && !isNaN(lldata.longitude)) {
          // 如果经纬度都成功数字化了
          wx.showLoading({
            title: "查询中",
            mask: true,
  ...
  async getLocation(data = null) {
    const { latitude, longitude } = data || that.data; // 如果传入为空，则使用data内数据
    await app
      .call({
        // 发起云函数请求
  ...
  async getAddress(address) {
    await app
      ...
      .then((res) => {
        // 请求成功
        that.setInfo([res.reallocal.lat, res.reallocal.lng], 0, {
          // 设置信息和地图中心点以及标记点
          info: res,
        });
      });
